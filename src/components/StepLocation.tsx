...
import { supabase } from "../utils/supabase/client"; // make sure this path matches your project

interface StepLocationProps {
  formData: any;
  setFormData: (d: any) => void;
  onNext?: () => void; // not used here; onFinish will be called
  onBack: () => void;
  onFinish: () => void; // parent will navigate to upload
}
...
function StepLocation({
  ...
  const handleFinish = async () => {
    if (!isValid) {
      toast.error("Please select at least one county or add a custom county.");
      return;
    }

    const services = customCounty.trim()
      ? [...selectedCounties, customCounty.trim()]
      : selectedCounties;
    const finalData = {
    ...
    setLoading(true);
    try {
      // get session to attach user_id if available
      ...
      if (userId) {
        // add user_id column if exists in your table
        (finalData as any).user_id = userId;
      }
      ...
      if (error) {
        ...
        toast.error("Failed to save profile: " + (error.message ?? "Unknown"));
      } else {
        toast.success("Business profile saved successfully!");
        // update parent formData (optional)
        setFormData({ ...formData, services, trade: formData.trade });
        // call parent finish handler (navigates to upload)
        onFinish();
      }
    } catch (e: any) {
